<div class="isiwork">
    ...
    <?php
        ...
        $sql = 'SELECT * FROM video ORDER BY ID ASC LIMIT '.$halaman1.', 5';
        $data = $koneksi->query($sql);

        // print_r($data->fetch_all());
        while($row = $data->fetch_assoc()) {
            echo '<div class="videoWrapper"><iframe width="560" height="315" src="'.$row['gambarvideo'].'" frameborder="0" allowfullscreen></iframe></div>';
            echo '<p><b>Name</b> | '.$row['namavideo'].'</p>';
            echo '<p><b>Platform</b> | '.$row['platformvideo'].'</p>';
            echo '<p><b>Software</b> | '.$row['softvideo'].'</p>';
            echo '<p><b>Date</b> | '.$row['datevideo'].'</p>';
            echo '<p><b>Category</b> | '.$row['kategorivideo'].'</p>';
            echo '<p><b>Link</b> | <i><u><a target="_blank" href="'.$row['linkvideo'].'">'.$row['namavideo'].'</a></u></i></p>';
        }
