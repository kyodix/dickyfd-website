<div class="isiwork">
    ...
    <img src="img/monitor icon.png" alt="">
    <h2>WEBSITE</h2>
    <?php
        include 'koneksi.php';
        ...
        $sql = 'SELECT * FROM website ORDER BY ID ASC LIMIT '.$halaman1.', 5';
        $data = $koneksi->query($sql);

        // print_r($data->fetch_all());
        while($row = $data->fetch_assoc()) {
            echo '<img src="'.$row['gambarwebsite'].'">';
            echo '<p><b>Name</b> | '.$row['namawebsite'].'</p>';
            echo '<p><b>Platform</b> | '.$row['platformwebsite'].'</p>';
            echo '<p><b>Software</b> | '.$row['softwebsite'].'</p>';
            echo '<p><b>Date</b> | '.$row['datewebsite'].'</p>';
            echo '<p><b>Category</b> | '.$row['kategoriwebsite'].'</p>';
            echo '<p><b>Link</b> | <i><u><a target="_blank" href="'.$row['linkwebsite'].'">'.$row['namawebsite'].'</a></u></i></p>';
        }
